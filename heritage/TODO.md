...
- [x] **Fix kart oversteering with YOLO pipeline** (2026-03-11) -- steering_gain halved to 0.5, exposed as `steering_gain` launch arg in autonomous.launch.py. Diagnostic test in tests/test_steering_gain.py.
- [x] **Remove raw value display from dashboard skins** (2026-03-11) -- all four skins (Default, KITT, Tesla, HUD) updated to show calibrated degrees only.
- [x] **Include manual remote control of the kart via the dashboard** (2026-03-11) -- Gamepad API polling at 10 Hz, PWM/Angle toggle, steering/throttle/brake bars. Publishes to /kart/cmd_vel_manual.
- [x] **Investigate rviz window accumulation on Orin** (2026-03-11) -- cleanup logic added to run_live.sh, display_zed_cam.launch.py, autonomous.launch.py, gui.launch.py. Diagnostic test in tests/test_rviz_accumulation.py.
