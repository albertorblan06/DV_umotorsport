...
def launch_setup(context, *args, **kwargs):
    ...
    # Kill stale rviz2 instances to prevent GPU resource accumulation on Orin
    cleanup_rviz = ExecuteProcess(
        cmd=[
            "bash",
            "-c",
            "killall -q rviz2 2>/dev/null; pkill -f rviz2 2>/dev/null; sleep 0.5; exit 0",
        ],
        name="cleanup_stale_rviz2",
        output="log",
    )
    ...
    return [cleanup_rviz, rviz2_node, zed_wrapper_launch]
